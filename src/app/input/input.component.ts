...
// Проще всего проверить как это работает:
...
// здесь на 73 строке сработает this._changeFn с введенным значением и твой текст передастся в formControl
...
export class InputComponent implements ControlValueAccessor, AfterViewInit, OnDestroy {
  ...
  // _changeFn - вызываем, когда хотим прокинуть наше значение наружу, то есть, чтобы из компонента изменить привязанный к нему formControl
